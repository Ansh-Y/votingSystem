...
-- Add triggers to automatically update poll status
DELIMITER //

-- Trigger to start polls when start_date is reached
CREATE TRIGGER update_poll_status_to_ongoing
...
-- Trigger to end polls when end_date is reached
CREATE TRIGGER update_poll_status_to_ended
BEFORE UPDATE ON polls
FOR EACH ROW
BEGIN
    IF NEW.status = 'ongoing' AND NEW.end_date <= NOW() THEN
...
-- Add a scheduled event to automatically update poll status
CREATE EVENT update_poll_statuses
ON SCHEDULE EVERY 10 MINUTE
DO
    UPDATE polls 
    SET status = 'ongoing' 
    WHERE status = 'pending' AND start_date <= NOW() AND end_date > NOW();

CREATE EVENT end_expired_polls
ON SCHEDULE EVERY 10 MINUTE
DO
    UPDATE polls 
    SET status = 'ended' 
